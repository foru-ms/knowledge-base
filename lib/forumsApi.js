/**
 * Server-side Knowledge Base API client
...
/**
 * Server-side Knowledge Base API Client
 * Maps knowledge base endpoints to backend API
 */
const knowledgeBaseApi = {
    ...
    // Article endpoints (formerly threads)
    articles: {
        async fetchAll(page = 1) {
        ...
        async fetchComments(articleId, page = 1) {
            return apiFetch(`/thread/${articleId}/posts?page=${page}`, {
                method: 'GET',
            });
        },
    },

    // Comment endpoints (formerly posts)
    comments: {
        async fetchAll(page = 1) {
        ...
        async create(body, articleId, userId) {
            return apiFetch('/post', {
                method: 'POST',
                body: JSON.stringify({ body, threadId: articleId, userId }),
            });
        },
    ...
    // Thread endpoints (deprecated, kept for compatibility)
    threads: {
        async fetchAll(page = 1) {
            return apiFetch(`/threads?page=${page}`, {
                method: 'GET',
    ...
    // Post endpoints (deprecated, kept for compatibility)
    posts: {
        async fetchAll(page = 1) {
            return apiFetch(`/posts?page=${page}`, {
                method: 'GET',
...
const forumsApi = knowledgeBaseApi;

export { knowledgeBaseApi, forumsApi, ApiError };
